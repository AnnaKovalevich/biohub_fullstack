...
export const projectRouter = trpc.router({
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (!ownerId) {
        ...
        if (!firstUser) {
          throw new TRPCError({
            code: "UNAUTHORIZED",
            message: "Нет пользователей. Зарегистрируйтесь.",
          });
        }
        ownerId = firstUser.id;
        console.log(
          `[createProject] Используем fallback-пользователя: ${ownerId}`,
        );
      }
  ...
  list: trpc.procedure.query(async ({ ctx }) => {
    ...
    const shared = await ctx.prisma.project.findMany({
      where: {
        sharedAccess: { some: { userId: ctx.userId, status: "accepted" } },
      },
      include: { files: true },
    });
    ...
    .query(async ({ input, ctx }) => {
      ...
      const hasAccess =
        ...
        project.sharedAccess.some(
          (sa) => sa.userId === ctx.userId && sa.status === "accepted",
        );
  ...
  update: trpc.procedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (input.experimentDate !== undefined)
        updateData.experimentDate = input.experimentDate
          ? new Date(input.experimentDate)
          : null;
      if (input.pipelineParams !== undefined)
        updateData.pipelineParams = input.pipelineParams; // <-- добавлено
